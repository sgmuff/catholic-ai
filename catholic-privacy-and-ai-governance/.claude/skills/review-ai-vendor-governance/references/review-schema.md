...
- `baseline_items` (array, required) — exactly one entry per item in
  `${CLAUDE_SKILL_DIR}/references/baseline/ai-vendor.md`
...
- `description` (string, required) — what the vendor's system does and
  what institutional data or decisions it touches.
- `service_provided` (string) — the specific model, system, or feature
  under contract.
...
`${CLAUDE_SKILL_DIR}/references/baseline/ai-vendor.md` — no more, no
fewer, no duplicates.
...
- `rationale` (string, required) — the specific baseline gaps and
  mitigating factors that drove the level.
