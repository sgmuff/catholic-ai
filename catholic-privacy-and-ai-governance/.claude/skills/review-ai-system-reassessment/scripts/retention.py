#!/usr/bin/env python3
"""Validates an AI-system-reassessment JSON against this skill's bundled
framework registry, then renders it to Markdown. The model does the
judgment — whether the system's current assessment is still current; this
script only checks the judgment is internally consistent. See SKILL.md
step 3.
...
class RetentionEntryError(Exception):
    ...
    def __init__(self, errors: list[str]) -> None:
        self.errors = errors
        super().__init__(
            "Could not build reassessment entry:\n" + "\n".join(f"- {e}" for e in errors)
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate and render an AI-system reassessment.")
    ...
    try:
        ...
        print(f"Could not build reassessment entry: {exc}", file=sys.stderr)
        return 1
