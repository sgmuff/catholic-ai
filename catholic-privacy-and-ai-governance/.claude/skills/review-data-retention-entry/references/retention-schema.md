...
This is the smallest schema in the family, deliberately: one inventory
entry, checked once, producing one verdict — not a score, not a list of
deadlines, not a checklist.

## Top level

- `title` (string, required) — a short, specific name for this entry
  (e.g. "Parish bulletin subscriber list"). Used to name the rendered
  report file.
...
- `description` (string, required) — what this inventory entry is.
- `category` (string, required) — the kind of thing this entry is (e.g.
  "data element", "processing activity").
- `purpose` (string, required) — why it exists.
- `last_reviewed_date` (string, required) — ISO date (`YYYY-MM-DD`) this
  entry was last reviewed or its retention justification last confirmed.
...
- `rationale` (string, required) — the specific facts that drove the
  verdict, grounded in the entry's own stated review interval and what
  `last_reviewed_date` shows about whether that interval has lapsed.
